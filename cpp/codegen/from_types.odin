...
type_to_type :: proc(
    ...
    case .Elaborated:
        tp = elaborated_to_type(type, name_hint) or_return
    case .Pointer:
        tp = pointer_to_type(type, cursor, type_hint, name_hint) or_return
    case .ConstantArray:
        tp = constant_array_to_type(
            type,
            cursor,
            type_hint,
            name_hint,
    ...
    case .Typedef:
        tp = typedef_to_type(type) or_return
    case .Record:
        tp = record_to_type(cursor) or_return
    case .Enum:
        tp = enum_to_type(cursor, name_hint) or_return
    case .FunctionNoProto, .FunctionProto:
        tp = function_to_type(type, cursor, name_hint) or_return
    case:
...
pointer_to_type :: proc(
    ...
    if clang.isConstQualifiedType(pointee) {
        if len(tp.array_info) != 0 {
            tp.array_info[len(tp.array_info) - 1].pointer_info.read_only = true
        } else {
...
record_to_type :: proc(
    ...
    clang.visitChildren(cursor, proc "c" (cursor, parent: clang.Cursor, client_data: clang.ClientData) -> clang.ChildVisitResult {
            ...
            if field_size := clang.getFieldDeclBitWidth(cursor); field_size != -1 {
                ...
                type, data.err = type_to_type(cursor_type, cursor, type_hint, member_name)
...
enum_to_type :: proc(
    ...
    case:
        ...
        err = errors.message("invalid enum type: {}", clang.getCString(spel))
        return
    }
...
function_to_type :: proc(
    ...
    func.variadic = bool(num_params != 0 && clang.isFunctionTypeVariadic(type))
    ...
    clang.visitChildren(cursor, proc "c" (cursor, parent: clang.Cursor, client_data: clang.ClientData) -> clang.ChildVisitResult {
            ...
            type: runic.Type = ---
            type, data.err = type_to_type(param_type, cursor, param_hint, param_name)
            if data.err != nil do return .Break
...
@(private)
handle_anon_type :: #force_inline proc(tp: ^runic.Type, prefix: string = "") {
    ctx := ps()
